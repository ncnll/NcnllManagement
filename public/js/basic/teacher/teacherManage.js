...
$(function(){
    ...
    $("#teacherGrid").datagrid({
        ...
        columns:[[
                  ...
                  {field:'fullName', title:'真实姓名', width:180},
                  {field:'email', title:'邮箱', width:180},
                  {field:'createDate', title:'创建日期', width:180},
                  {field:'operate', title:'操作', width:120, formatter: function(value,row,index){
                            if(row.isDisabled){
...
function newForm(){
    $('#dlg').dialog('open').dialog('setTitle','教师注册');
    $('#fm').form('clear');
...
function editForm(){
    selectedRow = $('#teacherGrid').datagrid('getSelected');

    //$('#usernameId').textbox('readonly',true);
   
    //$("#usernameId").textbox("readonly");
    if (selectedRow){
        $('#dlg').dialog('open').dialog('setTitle','教师修改');
